...
class CellNetwork(nx.classes.graph.Graph):
    ...
    def generate_shape(self, shape, n=1, m=1):
        ...
        self.set_edges()
        self.set_edges(attr=self.PD_attr)
    ...
    def set_edges(self, E=None, attr=None):
        if attr is None:
            attr = self.edge_attr
        if E is not None:
            self.update_edge_attribute(attr, E)
            return
    ...
    def get_weights(self, attr=None):
        if attr is None:
            attr = self.edge_attr
        return list(nx.get_edge_attributes(self, attr).values())
    ...
    def weights_to_A(self, attr=None):
        if attr is None:
            attr = self.edge_attr
        A = nx.to_numpy_array(self)
        W1 = np.triu(A)
